  ...
  return {
    ...
    link: function (scope, element, attrs, ctrl, transclude) {
      ...
      function getDaysInMonth () {
        const month = scope.currentViewDate.getMonth()
        const date = new Date(scope.currentViewDate.getFullYear(), month, 1)
        const days = []
      ...
      function convertToUTC (localdate) {
        const dateObj = getDateAndTime(localdate)
        const utcdate = new Date(dateObj.getUTCFullYear(), dateObj.getUTCMonth(), dateObj.getUTCDate(), dateObj.getUTCHours(), dateObj.getUTCMinutes())
        return utcdate
      }
      ...
      scope.selectDate = function (day) {
        if (scope.pickdate === 'true' && day.showday) {
          for (const number in scope.month) {
            const item = scope.month[number]
      ...
      scope.updateDate = function () {
        if (scope.localdate) {
          const newdate = getDateAndTime(scope.localdate)
      ...
      if (scope.picktime) {
        ...
        scope.initializeTimepicker = function () {
          ...
          sectionlength = timelineWidth / 24 / 6
        }
        ...
        angular.element($window).on('resize', function () {
          ...
          sectionlength = timelineWidth / 24
          scope.checkWidth(true)
        })

        scope.setTimeBar = function (date) {
          // currenttime = $('.current-time')
          // const timelineWidth = $('.timeline')[0].offsetWidth
          let hours = scope.time.split(':')[0]
          if (hours === 12) {
            hours = 0
          }
          const minutes = scope.time.split(':')[1]
          const minutesOffset = (minutes / 60) * (timelineWidth / 12)
          const hoursOffset = (hours / 12) * timelineWidth
          scope.currentoffset = parseInt(hoursOffset + minutesOffset - 1)
          currenttime.css({
        ...
        scope.getTime = function () {
          // get hours
          const percenttime = (scope.currentoffset + 1) / timelineWidth
        ...
        scope.timeSelectStart = function (event) {
          scope.initializeTimepicker()
          const timepickerContainer = element.find('.timepicker-container-inner')
          // timepickerContainer.offset().left
          ...
          scope.moving = true
          scope.currentoffset = scope.xinitial - timepickerContainer.offset().left
          scope.totaloffset = scope.xinitial - timepickerContainer.offset().left
          console.log(timepickerContainer.width())
          if (scope.currentoffset < 0) {
            scope.currentoffset = 0
          } else if (scope.currentoffset > timepickerContainer.width()) {
            scope.currentoffset = timepickerContainer.width()
          }
          currenttime.css({
            transform: 'translateX(' + scope.currentoffset + 'px)',
            transition: 'none',
        ...
        angular.element($window).on('mousemove touchmove', function (event) {
          if (scope.moving === true) {
            ...
            const movex = scope.offsetx + scope.totaloffset
            if (movex >= 0 && movex <= timelineWidth) {
              currenttime.css({
              ...
              scope.currentoffset = timelineWidth
            }
            scope.getTime()
            scope.$apply()
          }
        ...
        scope.adjustTime = function (direction) {
          ...
          if (newoffset < 0 || newoffset > timelineWidth) {
            if (newoffset < 0) {
              newoffset = timelineWidth - sectionlength
            } else if (newoffset > timelineWidth) {
